...
window.onload = () => {
    ...
    //Busqueda solo de la pagina cargada

    // Obtener referencia al campo de entrada de texto y al botón de búsqueda
    let searchInput = document.getElementById("searchInput");

    // Agregar evento 'input' al campo de entrada de texto
    searchInput.addEventListener('input', () => {
        let searchTerm = searchInput.value.trim().toLowerCase(); // Obtener el término de búsqueda y convertirlo a minúsculas
        
        // Filtrar los Pokémon cuyo nombre coincide con el término de búsqueda
        let filteredPokemon = Object.keys(pokemon).filter(pokemonName => {
            return pokemonName.toLowerCase().includes(searchTerm);
        });
        
        // Ocultar todos los Pokémon
        document.querySelectorAll("#containerpk article").forEach(pokemonElement => {
            pokemonElement.style.display = 'none';
        });
        
        // Mostrar solo los Pokémon filtrados
        filteredPokemon.forEach(pokemonName => {
